...
@section('navbar-row')
    <div class=" flex flex-col justify-center md:hidden">
        ...
        <div class="flex mr-2 space-x-3 ">
            ...
            <x-link-nav :href="route('purchases.cheque', ['purchaseId' => $purchase->id])" :active="request()->routeIs('purchases.cheque', ['purchaseId' => $purchase->id])" icon="fa-solid fa-money-bill">المعاملة</x-link-nav>
    ...
    <div class="flex flex-col h-full  ">
        ...
        <div class="flex-1 flex flex-col  overflow-y-auto custom-scroll md:items-center">
            <div class=" md:w-lg  lg:w-3xl">
                <form action="{{ route('purchases.cheque.storeCheque') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div>
                        @if (session('error'))
                    ...
                    <x-floatingLabelInput id="finalPrice" name="finalPrice" labelValue="المبلغ الكلي"
                        icon="fa-solid fa-money-bill" value="{{ $purchase->totalPrice }}" readonly />
                    ...
                    <x-floatingLabelInput id="cheque_remain" name="cheque_remain" labelValue="المبلغ المتبقي"
                        ...
                        value="{{ $purchase->totalPrice - $purchase->cheques->sum('amount') }}" />
