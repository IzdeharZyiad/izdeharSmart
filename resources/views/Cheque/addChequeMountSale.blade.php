...
@section('navbar-row')
    <div class=" flex flex-col justify-center md:hidden">
        ...
        <div class="flex mr-2 space-x-3 ">
            ...
            <x-link-nav :href="route('sales.cheque', ['saleId' => $cheque->chequeable->id])" :active="request()->routeIs('sales.cheque', ['saleId' => $cheque->chequeable->id])" icon="fa-solid fa-money-bill">المعاملة</x-link-nav>
    ...
    <div class="flex flex-col h-full  ">
        ...
        <div class="flex-1 flex flex-col  overflow-y-auto custom-scroll md:items-center">
            <div class=" md:w-lg  lg:w-3xl">
                <form action="{{ route('sales.storeChequeMount') }}" method="POST">
                    @csrf
                    <div>
                        @if (session('error'))
