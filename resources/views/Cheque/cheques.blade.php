        ...
        <div class=" mt-4 h-[40px]  ">
            ...
            <div class="grid grid-cols-[0.5fr_1fr] h-full">
                ...
                <div class="flex justify-end space-x-4  ml-2 ">
                    ...
                    @if ($purchase->cheques->sum('amount') != $purchase->totalPrice)
                        <a href="{{ route('purchases.cheque.addCheque', ['purchaseId' => $purchase->id]) }}"
        ...
        <div class="flex-1 h-full flex flex-col  mt-2 border-t border-gray-200  overflow-y-auto">
            ...
            <div class="flex flex-col md:grid grid-cols-3">
                <div class="border-b md:border-l  border-gray-200">
                    <x-card-nav icon="fa-solid fa-money-bill" text="مبلغ الشيكات" value="{{ $purchase->totalPrice }}  " />
                </div>
                <div class="border-b  md:border-l border-gray-200">
                    <x-card-nav icon="fa-solid fa-money-bill" text="المبلغ المدفوع"
                        value="{{ $purchase->transactions->sum('amount') }}  " />
                </div>

                <div class="border-b  md:border-l border-gray-200">
                    <x-card-nav icon="fa-solid fa-money-bill" text="المتبقي"
                        value="{{ $purchase->totalPrice - $purchase->transactions->sum('amount') }}   " />
                </div>
                            ...
                            <tr class="border-b border-gray-200">
                                ...
                                <td class="space-x-2">
                                    <x-link class="text-green-900"
                                        href="
                                    {{ route('purchases.addChequeMount', ['chequeId' => $cheque->id]) }}"
                                        value="دفع" />

                                    <x-link class="text-green-900"
                                        href="
                                    {{ route('purchases.chequeImg', ['chequeId' => $cheque->id]) }}"
                                        value="صورة الشيك" />
                                </td>
                            </tr>
                        @endforeach
