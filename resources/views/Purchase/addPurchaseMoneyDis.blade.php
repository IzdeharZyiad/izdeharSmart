        ...
        <div class="flex-1 flex flex-col  overflow-y-auto custom-scroll md:items-center">
            <div class=" md:w-lg  lg:w-3xl">
                <form action="{{ route('purchases.storePurchaseMoneyDis') }}" method="POST">
                    ...
                    <input type="hidden" name="purchaseDetail_id" value="{{ $purchaseDetail->id }}">


                    <x-floatingLabelInput id="totalPrice" name="totalPrice" labelValue="السعر الكلي الشراء"
                        icon="fa-solid fa-money-bill" value="{{ $purchaseDetail->subtotal }}" readonly />

                    <livewire:sale.discount :totalPrice="$purchaseDetail->subtotal" />
