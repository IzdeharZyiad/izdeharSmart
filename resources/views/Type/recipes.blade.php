...
@section('navbar-row')
    <div class=" flex flex-col justify-center md:hidden">
        <!-- center in sm -->
        <div class="flex mr-2 ">
            <x-link-nav :href="route('types.items.products.productDetails', [
                'itemId' => $productDetail->product->item_id,
                'productId' => $productDetail->product_id,
            ])" :active="request()->routeIs('types.items.products.productDetails', [
                'itemId' => $productDetail->product->item_id,
                'productId' => $productDetail->product_id,
            ])"
                icon="fa-solid fa-building">{{ $productDetail->product->name }}</x-link-nav>

            <x-link-nav icon="fa-solid fa-seedling">{{ $recipe->name }}</x-link-nav>
        </div>
        ...
        <div class=" mt-4 h-[40px]  ">
            ...
            <div class="grid grid-cols-[0.5fr_1fr] h-full">
                ...
                <div class="flex items-center mr-2">
                    <h1 class="text-green-900 text-sm font-medium">الوصفة </h1>
                </div>
                <div class="flex justify-end space-x-4  ml-2 ">
                    ...
                    <a href="{{ route('types.addRecipeDetail', ['recipeId' => $recipe->id]) }}"
                        ...
                        <span class="hidden md:block mr-2">اضافة ماده خام للوصفة</span>
                    </a>
        ...
        <div class="flex-1 h-full flex flex-col  mt-2 border-t border-gray-200  overflow-y-auto">
            ...
            <table id="myTable">
                ...
                <tbody>
                    ...
                    @foreach ($recipeDetials as $recipeDetial)
                        <tr class="border-b border-gray-200">
                            <td></td>
                            <td>{{ $recipeDetial->rawMaterial->name }}</td>
                            <td>{{ $recipeDetial->quantity }}</td>
                            <td>{{ $recipeDetial->rawMaterial->unit }}</td>
                            <td></td>
                        </tr>
                    @endforeach
