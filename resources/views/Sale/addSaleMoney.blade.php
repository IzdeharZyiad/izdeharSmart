    ...
    <div class="flex flex-col h-full  ">
        ...
        <div class="flex-1 flex flex-col  overflow-y-auto custom-scroll md:items-center">
            <div class=" md:w-lg  lg:w-3xl">
                <form action="{{ route('sales.storeSaleMoney') }}" method="POST">
                    @csrf
                    <div>
                        @if (session('error'))
                    ...
                    <x-floatingLabelInput id="totalPrice" name="totalPrice" labelValue="السعر الكلي للبيع"
                        icon="fa-solid fa-money-bill" value="{{ $saleDetail->sum('subtotal') }}" readonly />

                    <x-floatingLabelInput id="totalProfit" name="totalProfit" labelValue="الربح الكلي للبيع"
                        icon="fa-solid fa-money-bill" value="{{ $saleDetail->sum('profit') }}" readonly />
